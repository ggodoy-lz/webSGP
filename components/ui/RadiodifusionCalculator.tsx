...
export default function RadiodifusionCalculator({
  ...
  const canNext = (() => {
    if (paso === "medio") return !!medio;
    if (paso === "zona") return !!zona;
    if (paso === "categoria") return !!categoria;
    if (paso === "ingresos") return true; // 0 ingresos es válido → mínimo
    return false;
  })();
                    ...
                    {idx < stepLabels.length - 1 && (
                      <div
                        className={`flex-1 mx-3 h-px transition-colors ${
                          stepIdx > idx ? "bg-[#212226]/30" : "bg-[#212226]/8"
                        }`}
                      />
                    )}
                  </div>
