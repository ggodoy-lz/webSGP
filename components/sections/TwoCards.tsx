...
export default function TwoCards() {
  ...
  return (
    <section className="grid grid-cols-1 md:grid-cols-2">
      ...
      {/* Panel 2 — Licenciamiento destacado */}
      <motion.div
        initial={{ opacity: 0 }}
        whileInView={{ opacity: 1 }}
        viewport={{ once: true }}
        transition={{ delay: 0.1 }}
        className="group relative bg-[#4666a6] overflow-hidden"
      >
        <Link href={`/${locale}/licencias`} className="block p-10 lg:p-16 min-h-[480px] flex flex-col justify-between">
          {/* Ghost number */}
          <div className="text-outline text-white/10 font-display font-black text-[12rem] leading-none select-none absolute -top-4 -right-4 pointer-events-none">
            02
          </div>

          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#f2b33d] mb-8">02</p>
            <h2 className="font-display font-black text-white text-5xl lg:text-6xl xl:text-7xl leading-none mb-6">
              {t("licencia.title")}
            </h2>
          ...
          <div className="flex items-center gap-4 mt-10">
            <span className="text-xs font-black uppercase tracking-widest text-white border-b-2 border-[#f2b33d] pb-0.5 group-hover:text-[#f2b33d] transition-colors">
              {t("licencia.cta")}
            </span>
            <span className="text-[#f2b33d] text-lg group-hover:translate-x-2 transition-transform inline-block">
              →
            </span>
          </div>
        </Link>
