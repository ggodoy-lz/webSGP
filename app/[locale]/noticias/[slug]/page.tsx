...
export async function generateStaticParams() {
  // Prerenderiza lo que exista al momento del build. Las notas que SGP publique
  // después se renderizan a demanda, porque `dynamicParams` viene activo.
  return soloPublicadas(await leerNoticias()).map((a) => ({ slug: a.slug }));
}

async function buscarNota(slug: string) {
  return soloPublicadas(await leerNoticias()).find((a) => a.slug === slug);
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale, slug } = await params;
  const a = await buscarNota(slug);
  if (!a) return {};
  return {
    title: locale === "es" ? a.titleEs : a.titleEn,
    description: locale === "es" ? a.excerptEs : a.excerptEn,
  };
}
